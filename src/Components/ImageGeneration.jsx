...
import { generateImage, getBase64Response, uploadFileToIpfs } from "../utils";
...
const ImageGeneration = () => {
  ...
  const getIpfsHash = async () => {
    try {
      ...
      setImgHash(resJSON.data.IpfsHash);
    } catch (error) {
      alert(error);
    }
  };
  ...
  return (
    <div className="max-w-[100%] h-[64%] mx-auto mt-5 flex flex-row bg-black opacity-75 p-4">
      ...
      <div className="w-[50%] h-[70%] mx-1 bg-slate-700 rounded-[15px]  flex flex-col justify-center items-center gap-3">
        ...
        <div className="mt-2 px-auto bg-black w-full py-3 flex justify-center items-center gap-4">
          <button
            type="button"
          ...
          <a className="text-white " href={`https://ipfs.io/ipfs/${imgHash}`}>
          {imgHash && `https://ipfs.io/ipfs/${imgHash}`}</a>
        </div>
      </div>
    </div>
  );
